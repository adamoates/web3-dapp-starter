...
describe("Web3 Integration Tests (Simplified)", () => {
  ...
  describe("GET /api/web3/stats/:contractAddress", () => {
    it("should return contract stats from real MongoDB", async () => {
      jest.setTimeout(30000); // Extend timeout for this test

      const contractAddress = "0x" + "2".repeat(40); // Matches seeded data
      ...
      expect(response.body.stats).toHaveProperty("txCount");
      expect(response.body.stats).toHaveProperty("volume");
      expect(typeof response.body.stats.txCount).toBe("number");
      expect(typeof response.body.stats.volume).toBe("number");

      // Should have data from seeded blockchain events
      expect(response.body.stats.txCount).toBeGreaterThan(0);
    });

    it("should handle non-existent contract", async () => {
      const contractAddress = "0x" + "9".repeat(40); // Not in seeded data
      ...
      expect(response.body.stats.txCount).toBe(0);
    });
  });
});
